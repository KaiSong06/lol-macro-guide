...
Shipped so far: `capture`, `calibrate`, `detector`, `template_builder`, `riot_client`, `jungle_routes`, `state`, `config`, `logging_utils`, `benchmark`. Not yet in the repo: inference worker (Unit 7), `DecisionFilter` (Unit 8), TTS (Unit 9), main orchestrator (Unit 10), PyInstaller/NSIS packaging (Units 11–12). The `__main__.py` entry point is currently a stub. Phase 2 is blocked on a Windows benchmark gate run.
